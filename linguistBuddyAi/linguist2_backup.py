...
# --- LOGIC: GROQ ---
def _groq_worker(messages, model=MODEL_SMART):
    ...
    # 🧼 CLEANER: Remove the internal <think>...</think> blocks
    # This regex finds the "brain thoughts" and deletes them so you only see the final answer.
    clean_content = re.sub(r'<think>.*?</think>', '', raw_content, flags=re.DOTALL).strip()

    return clean_content

async def run_groq(messages, model=MODEL_SMART):
    return await asyncio.to_thread(_groq_worker, messages, model)
